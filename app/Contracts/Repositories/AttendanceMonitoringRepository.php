...
class AttendanceMonitoringRepository implements AttendanceMonitoringInterface
{
    ...
    public function getRecap(array $filters): array
    {
        $query = DB::table('attendances AS a')
            ->selectRaw('
                COALESCE(SUM(a.status = "hadir"), 0) AS total_hadir,
                COALESCE(SUM(a.status = "sakit"), 0) AS total_sakit,
                COALESCE(SUM(a.status = "izin"), 0) AS total_izin,
                COALESCE(SUM(a.status = "alpha"), 0) AS total_alpha
            ')
