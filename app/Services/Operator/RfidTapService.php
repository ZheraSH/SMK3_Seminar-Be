...
class RfidTapService
{
    ...
    //         $checkinStartCarbon = TapHelper::parseRuleTimeToCarbon($rule->checkin_start);
    //         $checkinEndCarbon = TapHelper::parseRuleTimeToCarbon($rule->checkin_end);
    //         $checkoutStartCarbon = TapHelper::parseRuleTimeToCarbon($rule->checkout_start);
    //         $checkoutEndCarbon = TapHelper::parseRuleTimeToCarbon($rule->checkout_end);

    //         $isCheckinTime = $checkinStartCarbon && $checkoutStartCarbon && $now->between($checkinStartCarbon, $checkoutStartCarbon->copy()->subSecond());
    //         $isCheckoutTime = $checkoutStartCarbon && $checkoutEndCarbon && $now->between($checkoutStartCarbon, $checkoutEndCarbon);

    //         if (!$isCheckinTime && !$isCheckoutTime) {
    //             $startDisplay = $checkinStartCarbon?->format('H:i') ?? '-';
    //             $status = $now->lessThan($checkinStartCarbon ?? $now) ? TapStatusEnum::BEFORE_TIME : TapStatusEnum::AFTER_TIME;
    //             $message = $status === TapStatusEnum::BEFORE_TIME ? 'Belum waktu absen. Jam absen masuk: ' . $startDisplay : 'Sudah lewat batas waktu absen';
    ...
    //         return $this->handleCheckout($student, $rfid, $rule, $now, $todayAttendance, $activeClassroom);
    //     });
    // }
    ...
    //     } elseif ($status === AttendanceStatusEnum::LATE->value) {
    //         $message = "Terlambat {$minutesLate} menit (masih dalam batas toleransi)";
    //     } else {
    //         $message = "Anda telat absen {$minutesLate} menit (melebihi toleransi) dan mendapatkan status Alpha";
    //     }

    //     return $this->successResponse(TapStatusEnum::VALID, $message, $student, $rfid, $attendance, TapTypeEnum::CHECKIN);
    // }

    // private function handleCheckout($student, $rfid, $rule, Carbon $now, $todayAttendance, $activeClassroom): array
    // {
    //     if (!$todayAttendance) {
    //         $data = [
    //             'student_id' => $student->id,
    ...
    //         $new = $this->attendanceRepository->store($data);
    //         $attendance = $this->attendanceRepository->show($new->id);
    //         return $this->successResponse(TapStatusEnum::VALID, 'Tap pulang dicatat, tapi status Alpha karena tidak absen masuk', $student, $rfid, $attendance, TapTypeEnum::CHECKOUT);
    //     }
    ...
    //         'attendance_status_label' => $attendance ? TapHelper::getSafeEnumLabel($attendance->status, \App\Enums\AttendanceStatusEnum::class) : null,
    ...
    //         'attendance_status_label' => TapHelper::getSafeEnumLabel($attendance->status, \App\Enums\AttendanceStatusEnum::class),
    //         'requires_manual_attendance' => false,
    //         'student' => $this->formatStudent($student),
    //         'rfid' => $rfid ? $this->formatRfid($rfid) : null,
    //         'attendance' => $this->formatAttendance($attendance),
